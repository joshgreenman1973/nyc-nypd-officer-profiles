...
**This file is generated by `build.py` from the 2026-08-02 export — do not edit by hand.**
...
|---|---|---|---|
| Members of Service (roster) | `pmsy-ewrc` | 33,819 | The officer list; all headline stats |
| Title / Shield History | `sh6y-4tgb` | 53,492 | Per-officer career timeline (fetched live) |
| Training | `n3mp-t5uj` | 12,721,262 | Aggregate training chart only |
| Department Recognition | `i9n8-a8ed` | 141,873 | Awards chart; "the decorated" |
| Disciplinary History — Summary | `wq9a-qu9a` | 1,510 | Recomputed here from the charges table |
| Disciplinary History — Charges | `uafj-ik29` | 3,840 | Discipline view; per-officer discipline |
...
- **`roster.json`** — all 33,819 officers as a compact columnar array. A
  per-officer count of sustained charges and a parsed precinct number are added.
- **`discipline.json`** — all 3,840 charges, each joined to the
...
- **Discipline is guilty findings only.** All 3,840 published
  charges carry a disposition of guilty, pleaded guilty or no contest
  (2,614 / 1,205 / 21 — which sum to 3,840). Dismissed,
...
- **The precinct map covers patrol precincts only.** Commands were mapped to a precinct by
  ...
  name because its command carries no number. About 14,219 of
  33,819 officers sit in a patrol precinct; the rest (housing, transit,
...
- **Awards given vs officers who hold them.** A single officer can receive the same award
  ...
  Police Duty, for example, is awarded 91,629 times but to 16,429
  distinct officers (about 5.6 each); Meritorious Police Duty 47,009 times to
  10,699 officers. The awards table shows both columns. The six top medals are
  almost exactly one per officer.
- **Two ways to count recognitions.** The awards table and the "recognitions awarded"
  headline (141,873) count individual award records in
  `i9n8-a8ed`. The roster also carries a per-officer recognition counter
  (`department_recognitions`) that sums to 145,431 — slightly
...
33,819 needs context before it is cited:

- **Recruits are in the count.** 1,076 officers are in the `RTS RECRUITS`
...
- **So are officers who aren't working.** 816 sit at the
...
- **Patrol strength is not a precinct's full footprint.** The precinct map counts patrol
  and field-training commands (14,219 officers). Another
  ~1,482 work in precinct-based detective and domestic-violence
