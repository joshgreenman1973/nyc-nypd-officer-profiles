...
**This file is generated by `build.py` from the 2026-07-12 export — do not edit by hand.**
...
|---|---|---|---|
| Members of Service (roster) | `pmsy-ewrc` | 34,184 | The officer list; all headline stats |
| Title / Shield History | `sh6y-4tgb` | 53,266 | Per-officer career timeline (fetched live) |
| Training | `n3mp-t5uj` | 12,782,034 | Aggregate training chart only |
| Department Recognition | `i9n8-a8ed` | 141,476 | Awards chart; "the decorated" |
| Disciplinary History — Summary | `wq9a-qu9a` | 1,535 | Recomputed here from the charges table |
| Disciplinary History — Charges | `uafj-ik29` | 3,885 | Discipline view; per-officer discipline |
...
- **`roster.json`** — all 34,184 officers as a compact columnar array. A
  per-officer count of sustained charges and a parsed precinct number are added.
- **`discipline.json`** — all 3,885 charges, each joined to the
...
- **Discipline is guilty findings only.** All 3,885 published
  charges carry a disposition of guilty, pleaded guilty or no contest
  (2,648 / 1,216 / 21 — which sum to 3,885). Dismissed,
...
- **The precinct map covers patrol precincts only.** Commands were mapped to a precinct by
  ...
  name because its command carries no number. About 14,203 of
  34,184 officers sit in a patrol precinct; the rest (housing, transit,
...
- **Awards given vs officers who hold them.** A single officer can receive the same award
  ...
  Police Duty, for example, is awarded 91,487 times but to 16,463
  distinct officers (about 5.6 each); Meritorious Police Duty 46,769 times to
  10,685 officers. The awards table shows both columns. The six top medals are
  almost exactly one per officer.
- **Two ways to count recognitions.** The awards table and the "recognitions awarded"
  headline (141,476) count individual award records in
  `i9n8-a8ed`. The roster also carries a per-officer recognition counter
  (`department_recognitions`) that sums to 144,760 — slightly
...
34,184 needs context before it is cited:

- **Recruits are in the count.** 1,135 officers are in the `RTS RECRUITS`
  command — recruits in the Police Academy, and the single largest command on the whole
  force — plus about 126 in the Police Academy itself. They are active
  members but not deployed.
- **So are officers who aren't working.** 777 sit at the
  ...
  32,272 than 34,184.
- **Domestic-violence officers moved out of patrol.** About 502 officers
...
- **Patrol strength is not a precinct's full footprint.** The precinct map counts patrol
  and field-training commands (14,203 officers). Another
  ~1,474 work in precinct-based detective and domestic-violence
