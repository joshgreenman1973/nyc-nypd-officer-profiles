...
**This file is generated by `build.py` from the 2026-09-13 export — do not edit by hand.**
...
|---|---|---|---|
| Members of Service (roster) | `pmsy-ewrc` | 34,105 | The officer list; all headline stats |
| Title / Shield History | `sh6y-4tgb` | 53,620 | Per-officer career timeline (fetched live) |
| Training | `n3mp-t5uj` | 12,927,005 | Aggregate training chart only |
| Department Recognition | `i9n8-a8ed` | 142,734 | Awards chart; "the decorated" |
| Disciplinary History — Summary | `wq9a-qu9a` | 1,475 | Recomputed here from the charges table |
| Disciplinary History — Charges | `uafj-ik29` | 3,744 | Discipline view; per-officer discipline |
...
| Dataset | ID | Rows | Role in this site |
|---|---|---|---|
| Allegations Against Police Officers | `6xgr-kwjq` | 433,020 | Complaint counts per officer; the FADO and disposition charts; per-officer detail (fetched live) |
| Police Officers | `2fir-qns4` | 98,293 | The name-and-shield match to the NYPD roster |
| Complaints Against Police Officers | `2mby-ccnw` | 141,821 | Incident date, precinct and body-camera evidence |
| Penalties | `keep-pkmh` | 15,241 | What the NYPD did after the board substantiated |
...
when unique. 33,525 of 34,105 serving officers
...
- **`roster.json`** — all 34,105 officers as a compact columnar array. A
  per-officer count of sustained charges and a parsed precinct number are added.
- **`discipline.json`** — all 3,744 charges, each joined to the
...
- **`ccrb.json`** — per-officer complaint counts for the 18,822
...
- **An allegation is an account, not a finding.** 433,020 allegations are on
  file; 26,651 (6.2%) were substantiated by
...
- **Substantiated is not punished.** The board investigates; the NYPD decides the penalty and
  reports it back. Across 15,241 referrals, the most common single outcome
...
- **Only serving officers appear.** The complaint tables cover
  ...
  18,822 who are still on today's roster, so an officer who
...
- **The complaint map is by place, not by roster.** Allegations are counted where the incident
  occurred. 9,087 allegations carry no precinct and are left
...
- **Discipline is guilty findings only.** All 3,744 published
  charges carry a disposition of guilty, pleaded guilty or no contest
  (2,542 / 1,185 / 17 — which sum to 3,744). Dismissed,
...
- **The precinct map covers patrol precincts only.** Commands were mapped to a precinct by
  ...
  name because its command carries no number. About 13,877 of
  34,105 officers sit in a patrol precinct; the rest (housing, transit,
...
- **Awards given vs officers who hold them.** A single officer can receive the same award
  ...
  Police Duty, for example, is awarded 92,190 times but to 16,498
  distinct officers (about 5.6 each); Meritorious Police Duty 47,360 times to
...
- **Two ways to count recognitions.** The awards table and the "recognitions awarded"
  headline (142,734) count individual award records in
  `i9n8-a8ed`. The roster also carries a per-officer recognition counter
  (`department_recognitions`) that sums to 145,967 — slightly
...
34,105 needs context before it is cited:

- **Recruits are in the count.** 1,790 officers are in the `RTS RECRUITS`
  command — recruits in the Police Academy, and the single largest command on the whole
  force — plus about 111 in the Police Academy itself. They are active
  members but not deployed.
- **So are officers who aren't working.** 821 sit at the
  ...
  31,494 than 34,105.
- **Domestic-violence officers moved out of patrol.** About 488 officers
  hold domestic-violence assignments, 446 of them in
...
- **Patrol strength is not a precinct's full footprint.** The precinct map counts patrol
  and field-training commands (13,877 officers). Another
  ~1,436 work in precinct-based detective and domestic-violence
