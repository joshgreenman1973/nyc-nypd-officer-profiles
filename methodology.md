...
**This file is generated by `build.py` from the 2026-08-23 export — do not edit by hand.**
...
|---|---|---|---|
| Members of Service (roster) | `pmsy-ewrc` | 34,239 | The officer list; all headline stats |
| Title / Shield History | `sh6y-4tgb` | 53,297 | Per-officer career timeline (fetched live) |
| Training | `n3mp-t5uj` | 12,733,738 | Aggregate training chart only |
| Department Recognition | `i9n8-a8ed` | 142,540 | Awards chart; "the decorated" |
| Disciplinary History — Summary | `wq9a-qu9a` | 1,492 | Recomputed here from the charges table |
| Disciplinary History — Charges | `uafj-ik29` | 3,787 | Discipline view; per-officer discipline |
...
when unique. 33,033 of 34,239 serving officers
...
- **`roster.json`** — all 34,239 officers as a compact columnar array. A
  per-officer count of sustained charges and a parsed precinct number are added.
- **`discipline.json`** — all 3,787 charges, each joined to the
...
- **`ccrb.json`** — per-officer complaint counts for the 18,783
...
- **Only serving officers appear.** The complaint tables cover
  ...
  18,783 who are still on today's roster, so an officer who
...
- **Discipline is guilty findings only.** All 3,787 published
  charges carry a disposition of guilty, pleaded guilty or no contest
  (2,597 / 1,173 / 17 — which sum to 3,787). Dismissed,
...
- **The precinct map covers patrol precincts only.** Commands were mapped to a precinct by
  ...
  name because its command carries no number. About 13,951 of
  34,239 officers sit in a patrol precinct; the rest (housing, transit,
...
- **Awards given vs officers who hold them.** A single officer can receive the same award
  ...
  Police Duty, for example, is awarded 91,982 times but to 16,481
  distinct officers (about 5.6 each); Meritorious Police Duty 47,344 times to
  10,747 officers. The awards table shows both columns. The six top medals are
  almost exactly one per officer.
- **Two ways to count recognitions.** The awards table and the "recognitions awarded"
  headline (142,540) count individual award records in
  `i9n8-a8ed`. The roster also carries a per-officer recognition counter
  (`department_recognitions`) that sums to 145,836 — slightly
...
34,239 needs context before it is cited:

- **Recruits are in the count.** 1,798 officers are in the `RTS RECRUITS`
...
- **So are officers who aren't working.** 821 sit at the
  ...
  31,620 than 34,239.
- **Domestic-violence officers moved out of patrol.** About 493 officers
  hold domestic-violence assignments, 452 of them in
...
- **Patrol strength is not a precinct's full footprint.** The precinct map counts patrol
  and field-training commands (13,951 officers). Another
  ~1,455 work in precinct-based detective and domestic-violence
