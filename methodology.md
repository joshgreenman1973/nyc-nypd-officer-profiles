...
**This file is generated by `build.py` from the 2026-08-16 export — do not edit by hand.**
...
|---|---|---|---|
| Members of Service (roster) | `pmsy-ewrc` | 34,282 | The officer list; all headline stats |
| Title / Shield History | `sh6y-4tgb` | 53,374 | Per-officer career timeline (fetched live) |
| Training | `n3mp-t5uj` | 12,736,272 | Aggregate training chart only |
| Department Recognition | `i9n8-a8ed` | 142,533 | Awards chart; "the decorated" |
| Disciplinary History — Summary | `wq9a-qu9a` | 1,499 | Recomputed here from the charges table |
| Disciplinary History — Charges | `uafj-ik29` | 3,813 | Discipline view; per-officer discipline |
...
- **`roster.json`** — all 34,282 officers as a compact columnar array. A
  per-officer count of sustained charges and a parsed precinct number are added.
- **`discipline.json`** — all 3,813 charges, each joined to the
...
- **Discipline is guilty findings only.** All 3,813 published
  charges carry a disposition of guilty, pleaded guilty or no contest
  (2,594 / 1,202 / 17 — which sum to 3,813). Dismissed,
...
- **The precinct map covers patrol precincts only.** Commands were mapped to a precinct by
  ...
  name because its command carries no number. About 14,132 of
  34,282 officers sit in a patrol precinct; the rest (housing, transit,
...
- **Awards given vs officers who hold them.** A single officer can receive the same award
  ...
  Police Duty, for example, is awarded 91,970 times but to 16,479
  distinct officers (about 5.6 each); Meritorious Police Duty 47,345 times to
  10,745 officers. The awards table shows both columns. The six top medals are
  almost exactly one per officer.
- **Two ways to count recognitions.** The awards table and the "recognitions awarded"
  headline (142,533) count individual award records in
  `i9n8-a8ed`. The roster also carries a per-officer recognition counter
  (`department_recognitions`) that sums to 145,681 — slightly
...
34,282 needs context before it is cited:

- **Recruits are in the count.** 1,801 officers are in the `RTS RECRUITS`
  command — recruits in the Police Academy, and the single largest command on the whole
  force — plus about 119 in the Police Academy itself. They are active
  members but not deployed.
- **So are officers who aren't working.** 812 sit at the
...
- **Patrol strength is not a precinct's full footprint.** The precinct map counts patrol
  and field-training commands (14,132 officers). Another
  ~1,456 work in precinct-based detective and domestic-violence
